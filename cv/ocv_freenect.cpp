...
int main(int argc, char **argv)
{
	...
	int latestStreamId = 0;
	
	Point2i appear(-1,-1);	
	Point2i midBlob(-1,-1);
	Point2i lastMove(-1,-1);
	
	vector<int> hc_stack(20);	
	while (!die) {
		fr++;
		...
		// cout << "         x=" << blb[0] << ",y=" << blb[1] << ",size=" << blb[2] << endl;
		...
		if(blb[0]>=0 && blb[2] > 500) { //1st blob detected, and is big enough
			...
			if(blb[0] >= 0 && blb[2] > 300) {
				...
				//cout << "appear:(x=" << appear.x << ",y=" << appear.y << "), blb:[0=" << blb[0] << ",1=" << blb[1] << "]" << endl;
				...
				if(appear.x < 0) {
					...
					latestStreamId++;					
					stringstream ss;
					ss << "\"x\":" << (FREENECT_FRAME_W - midBlob.x) << ",\"y\":" << midBlob.y << ",\"streamId\":" << latestStreamId << ",\"ts\":" << getTickCount();
					send_event("HandStart", ss.str());
					//cout << ">>> start x=" << midBlob.x << ",y=" << midBlob.y << endl;
					appear = midBlob;					
				}
				else {
					if (abs(appear.x - midBlob.x) > 100 || abs(appear.y - midBlob.y) > 100) {
						// oops, send end
						stringstream ss;
						ss << "\"x\":" << (FREENECT_FRAME_W - appear.x) << ",\"y\":" << appear.y << ",\"streamId\":" << latestStreamId << ",\"ts\":" << getTickCount();					
						send_event("HandEnd", ss.str());
						//cout << "<<< end[1]  x=" << appear.x << ",y=" << appear.y << endl;
						appear.x = -1;
					}
					else if (appear.x != midBlob.x || appear.y != midBlob.y) {
						// move
						stringstream ss;
						ss << "\"x\":" << (FREENECT_FRAME_W - midBlob.x) << ",\"y\":" << midBlob.y << ",\"streamId\":" << latestStreamId << ",\"ts\":" << getTickCount();
						send_event("HandMove", ss.str());
						appear = midBlob;						
					}
				}
				send_image(outC);
				...
				ss << "\"x\":" << (FREENECT_FRAME_W - appear.x) << ",\"y\":" << appear.y << ",\"streamId\":" << latestStreamId << ",\"ts\":" << getTickCount();					
				send_event("HandEnd", ss.str());
				//cout << "<<< end[2]  x=" << appear.x << ",y=" << appear.y << endl;
				appear.x = -1;
			}
		}
		else if (appear.x >= 0) {
			...
			ss << "\"x\":" << (FREENECT_FRAME_W - appear.x) << ",\"y\":" << appear.y << ",\"streamId\":" << latestStreamId << ",\"ts\":" << getTickCount();					
			send_event("HandEnd", ss.str());
			//cout << "<<< end[3]  x=" << appear.x << ",y=" << appear.y << endl;
			appear.x = -1;			
		}
